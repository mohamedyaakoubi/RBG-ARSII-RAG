...
def create_embeddings_table():
    ...
    try:
        ...
        cursor.execute(f"""
                CREATE TABLE IF NOT EXISTS legacy_embeddings (
...
def clear_embeddings_table():
    ...
    try:
        cursor = connection.cursor()
        cursor.execute("TRUNCATE TABLE legacy_embeddings RESTART IDENTITY")
        connection.commit()
        cursor.close()
        close_db_connection(connection)
...
def insert_embedding(id_document, texte_fragment, vecteur):
    ...
    try:
        cursor = connection.cursor()
        cursor.execute("""
            INSERT INTO legacy_embeddings (id_document, texte_fragment, vecteur)
            VALUES (%s, %s, %s)
